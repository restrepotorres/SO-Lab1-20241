...
int main(int argc, char *argv[]) {
  ...
  //Error si el archivo de entrada y de salida son el mismo
  if (argc == 3) {
    if (strcmp(argv[1], argv[2]) == 0) {
    printf("Error: Los argumentos de entrada y salida no pueden ser iguales\n");
    return 1; 
}
  }
  ...
  while (fgets(buffer, sizeof(buffer), fp) != NULL) {
    ...
    // Asignar memoria para la línea actual, y se le suma el espacio para un 2
    // char para poder ponerle
    // \n o \0, es decir un salto de linea o un simbolo de terminacion de
    // archivo
  ...
  // Si 3 argumentos, entonces se guarda el resultado en el archivo del tercer
  // argumento sino, se muestra en la salida estandar
  FILE *output_fp;
